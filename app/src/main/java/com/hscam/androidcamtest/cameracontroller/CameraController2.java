...
public class CameraController2 extends CameraController {
	...
	private Object image_reader_lock = new Object();
	...
	private boolean use_fake_precapture = false;
	...
	public CameraFeatures getCameraFeatures() {
		...
		for(int i=0;i<face_modes.length;i++) {
			...
			if( face_modes[i] == CameraCharacteristics.STATISTICS_FACE_DETECT_MODE_FULL || face_modes[i] == CameraCharacteristics.STATISTICS_FACE_DETECT_MODE_SIMPLE) {
	...
	private Point convertPointFromCamera2(Rect crop_rect, Point camera2_point){
		double x_f = (camera2_point.x-crop_rect.left)/(double)(crop_rect.width()-1);
		double y_f = (camera2_point.y-crop_rect.top)/(double)(crop_rect.height()-1);

		int x = (int)(x_f * 2000) - 1000;
		int y = (int)(y_f * 2000) - 1000;
			...
			if( face_detection_listener != null && previewBuilder != null && previewBuilder.get(CaptureRequest.STATISTICS_FACE_DETECT_MODE) != null && previewBuilder.get(CaptureRequest.STATISTICS_FACE_DETECT_MODE) == CaptureRequest.STATISTICS_FACE_DETECT_MODE_FULL ) {
				...
				if( camera_faces != null ) {
					...
					for(int i=0;i<camera_faces.length;i++) {
						...
						faces[i].leftEye = camera_faces[i].getLeftEyePosition();
						faces[i].rightEye = camera_faces[i].getRightEyePosition();
						faces[i].mouth = camera_faces[i].getMouthPosition();
					}
